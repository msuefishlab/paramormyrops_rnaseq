...
genes.of.interest <- function (groups, threshold) {
  ...
  #get mean values of each group
  av.values.g1 <- apply(data[,groups[[1]]], 1, FUN = mean)
  av.values.g2 <- apply(data[,groups[[2]]], 1, FUN = mean)
  ...
  #add a columns with the desired differences (see below). This will allow to rank the selected genes 
  data.mod <- cbind((av.values.g1 - av.values.g2), (av.values.g2 - av.values.g1), data)
  colnames(data.mod)[c(1,2)] <- c("g1", "g2")
  
 # genes enriched in group#1 (=depleted in group#2): mean expression value of group#1 is t (threshold) larger than the mean expression value of group#2
  dummy <- data.mod[data.mod[, "g1"] >= threshold, , drop = FALSE]
  #rank the genes
  dummy <- dummy[order(dummy[,"g1"], decreasing = TRUE),]
  #delete the columns with the differences in mean values
  result[[2]] <- dummy[,-1:-2, drop = FALSE]
  
  
  # genes enriched in group#2 (=depleted in group#1): mean expression value of group#2 is t (threshold) larger than the mean expression value of group#1
  dummy <- data.mod[data.mod[, "g2"] >= threshold, , drop = FALSE]
  #rank the genes
  dummy <- dummy[order(dummy[,"g2"], decreasing = TRUE),]
  
  #delete the columns with the differences in mean values
  result[[3]] <- dummy[,-1:-2, drop = FALSE]
...
#Maximum is fixed at 6.5, this covers every genes but two from smaller penetrations (otherwise most plots get a large empty space on top. Traeger's plots are cut at the top as well)
ymax <- 6.5
